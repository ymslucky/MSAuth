...
export function registerRoleRoutes(): Hono<ApiEnv> {
	...
	routes.post("/", requirePermission("roles:write"), async (c) => {
		...
		try {
			const role = await c.env.AUTH_DB
				.prepare("INSERT INTO role (name, description) VALUES (?1, ?2) RETURNING id")
				.bind(name, description)
				.first<{ id: string }>();
			await setRolePermissions(c.env.AUTH_DB, role!.id, permissions);
			await writeAudit(c.env.AUTH_DB, c.get("userId"), "role.create", "role", role!.id, JSON.stringify({ name }));
	...
	routes.patch("/:id", requirePermission("roles:write"), async (c) => {
		...
		// System roles keep their name: it is what the bootstrap and built-in
		// protections key on.
		...
		await writeAudit(db, c.get("userId"), "role.update", "role", id, JSON.stringify({ name, description, permissions }));
		return json({ ok: true });
	});

	routes.delete("/:id", requirePermission("roles:write"), async (c) => {
		const id = c.req.param("id");
		const db = c.env.AUTH_DB;
		const existing = await db
			.prepare("SELECT id, name, is_system FROM role WHERE id = ?1")
		...
		await writeAudit(db, c.get("userId"), "role.delete", "role", id, JSON.stringify({ name: existing.name }));
		return json({ ok: true });
	});
